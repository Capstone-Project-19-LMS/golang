...
func (cr *courseRepository) GetCourseEnrollByID(id string) ([]dto.CustomerEnroll, error) {
	...
	err := cr.db.Model(&model.Customer{}).Select("*","customer_courses.status AS status_enroll").Joins("JOIN customer_courses ON customer_courses.customer_id = customers.id").Where("customer_courses.course_id = ? ", id).Find(&customers)
	if err.Error != nil {
